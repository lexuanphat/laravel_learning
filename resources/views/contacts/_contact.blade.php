<tr>
    ...
    <td width="150">
        @if ($showTrashedButton)
            <form action="{{ route('contacts.restore', $contact->id) }}" style="display:inline" method="post">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-sm btn-circle btn-outline-info" title="Delete"><i
                        class="fa fa-undo"></i></button>
            </form>
            <form action="{{ route('contacts.force-delete', $contact->id) }}"
                onsubmit="return confirm('Are you sure you want to delete?')"style="display:inline" method="post">
                @csrf
                @method('DELETE')
